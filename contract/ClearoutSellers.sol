...
contract ClearoutSellers is multiowned {
    ...
    mapping(address=>_Power) maxPower; // record the max power to be able to generate by a meter
    ...
    function setMeterOwner(address _meter, address _owner, uint256 _maxElectricity, uint256 _type) onlymanyowners(sha3(msg.data)) returns (bool) {
        ownMeters[_owner].push( _meter);
        isMeter[_meter] = true;
        if (1 == _type ) {
            maxPower[_meter].peak = _maxElectricity;
        } else if (2 == _type) {
            maxPower[_meter].valley = _maxElectricity;
        } else if (3 == _type) {
            maxPower[_meter].flat = _maxElectricity;
        }
        return true;
    }
    ...
    function submitPower(uint256 _power, uint256 _type, uint256 _seqMeter, uint256 _seq, address _owner) returns (bool) {
        require(!isMeter[msg.sender] ) ;
        require(ownMeters[_owner][_seqMeter] != msg.sender) ;
    ...
    function presale(uint256 _electricity, uint256 _type, address _meter, uint256 _seq, uint256 _seqMeter, address _owner)  returns (bool) {
        require(!isMeter[_meter]) ;
        require(ownMeters[_owner][_seqMeter] != _meter) ;

        if (1== _type  ) {
            require(maxPower[_meter].peak >= _electricity);
            powers[_meter][_seq%numAsset].peak = _electricity;
            Presale(_electricity,_type, _meter);
            return true;
        } else if ( 2== _type ) {
            require(maxPower[_meter].valley >= _electricity);
            powers[_meter][_seq%numAsset].valley = _electricity;
            Presale(_electricity,_type, _meter);
            return true;
        } else if ( 3== _type ) {
            require( maxPower[_meter].flat >= _electricity);
