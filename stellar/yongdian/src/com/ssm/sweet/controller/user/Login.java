...
public class Login {
    ...
    public void login(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            if(bool){
                ...
                String[] info = getUserInfo(key);
                jsonObject.put("Balance",info[0]);
                jsonObject.put("Limit",info[1]);
                jsonObject.put("status","OK");
                KeyPair keyPair = KeyPair.fromSecretSeed(key);
                jsonObject.put("gkey",keyPair.getAccountId());
    ...
    public static String[] getUserInfo(String skey){
        String[] str = new String[2];
        Server server = new Server("https://horizon-testnet.stellar.org");
        //KeyPair trustor = KeyPair.fromSecretSeed("SCZANGBA5YHTNYVVV4C3U252E2B6P6F5T3U6MM63WBSBZATAQI3EBTQ4");//SCZANGBA5YHTNYVVV4C3U252E2B6P6F5T3U6MM63WBSBZATAQI3EBTQ4
        KeyPair trustor = KeyPair.fromSecretSeed(skey);//SCZANGBA5YHTNYVVV4C3U252E2B6P6F5T3U6MM63WBSBZATAQI3EBTQ4
        AccountResponse account = null;
        try {
            account = server.accounts().account(trustor);
        } catch (IOException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
        System.out.println("Balances for account " + trustor);
        for (AccountResponse.Balance balance : account.getBalances()) {
            ...
            if(balance.getAssetType().equals("native")){
                str[0]=balance.getBalance();
            }
            if(balance.getAssetCode()!=null&&balance.getAssetCode().equals("PING")){
                str[1]=balance.getLimit();
            }
        }
        return str;
